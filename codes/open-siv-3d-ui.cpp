...
void StageUI::update(double deltaTime, double gameTimeScale, Vec2 scroll, double resources, bool& canvasOpen, std::function<void(std::type_index)> onChangeScene) const
{
    // ゲームオーバー・ゲームクリアと通常プレイの画面用のスクロール値計算
    const auto ratio = (m_context->State() == GameCore::Playing) ? 0 : 1;

    // 現在, 目標値, 経過時間*8(通常、1秒の8分の1)でLerp
    ...
    // 通常プレイ画面を描画。左右で分けてある
    updateLeftSide(deltaTime, gameTimeScale, resources, canvasOpen);
...
void StageUI::updateLeftSide(double deltaTime, double gameTimeScale, double resources, bool& canvasOpen) const
{
    ...
    // 状態の更新。この中にdrawも入ってる。
    updateKimeraCanvas(deltaTime, gameTimeScale, canvasOpen);
...
void KirimiButton::draw(GameCore::SpriteAnimation animation, bool selected, double resource) const
{
    ...
    if (selected)
    {
        fill = m_activeColor;             // 選択時
    }
    else if (m_mouseOver)
    {
        fill = m_disableColor;            // ホバー時
    }
    ...
    // アイコンの描画(今回gif画像を使っているのでちょっと表示方法が特殊。)
    animation.Draw(center - size / 2, size);
...
{
    // 影。全体で影オフセットを統一してある。
    RoundRect{ 0, 0, 350, 900, 50 }
        .moveBy(m_shadowOffset)
        .draw(m_shadowColor);
}
...
void StageUI::drawKirimiPalette(double resources) const
{
    ...
    RoundRect{ 0, 0, 350, 900, 50 }
        .draw(m_baseColor)
        .drawFrame(0, 15, m_subColor);

    // 現在コストの座布団
    RoundRect{ 30, 800, 290, 75, 20 }
        .draw(m_baseColor)
        .drawFrame(0, 10, m_subColor);

    // 現在コスト(多分書き方ミスってる)
    const auto resText = Format(U"{:0>9}", static_cast<int>(resources));
    m_resourceLabel(resText).drawAt(Vec2{ 174, 835 }, m_subColor);
}
...
StageUI::CanvasRects StageUI::drawKimeraCanvas(Size size) const
{
    ...
    // フェードを描画
    Rect{ 0, 0, size.x, fadeLength }
        .draw(Arg::top(m_canvasFadeOutColor), Arg::bottom(m_canvasFadeInColor));

    Rect{ 0, size.y - fadeLength, size.x, fadeLength }
        .draw(Arg::top(m_canvasFadeInColor), Arg::bottom(m_canvasFadeOutColor));

    // 原点のリセット
    auto M = Graphics2D::GetLocalTransform();
    const Vec2 absOffset{ M._31, M._32 };

    rr.setPos(static_cast<int32>(absOffset.x), static_cast<int32>(absOffset.y));
    ...
    {
        // 詳細ボタン座布団の描画
        const auto r = Circle{ { x, 65 }, 36 }
            .draw(m_shadowColor)
            .movedBy(-propShadowOffset)
            .draw(m_mainColor);
    ...
    // 召喚ボタンの座布団描画
    result.spawnButton =
        RoundRect{ { 650, 700 }, { 350, 125 }, 20 }
            .draw(m_shadowColor)
            .movedBy(-propShadowOffset * 2)
            .draw(m_accentColor)
            .drawFrame(15, 0, m_subColor);

    // 詳細ボタンのアイコン描画
    const std::array<std::u32string, 4> iconList =
    {
        U"\U000F0415",
        U"\U000F0374",
        U"\U000F0467",
    ...
    // 各種インフォテキスト(未実装)
    m_infoLabel(U"キリミコスト x0123.45").draw(680, 25);
    m_infoLabel(U"キメラコスト 012345").draw(680, 75);
    m_alertLabel(U"コスト超過").drawAt(size / 2, ColorF{ Palette::Red, 0.75f });
...
void StageUI::drawChart(int size) const
{
    ...
    frameRect.drawFrame(0, outFrameSize, m_subColor); // 枠線
    ...
    // レーダーチャート用の計算・描画
    const Array<double> ratios = { 1.0, 0.8, 0.9, 0.1, 0.0, 0.05 };
    const Array<String> labels = { U"Hp", U"Atk", U"AtkRt", U"Dif", U"Spd", U"Wgt" };

    Array<Vec2> points;

    for (size_t i = 0; i < ratios.size(); ++i)
    {
        const double angle = Math::ToRadians(-90.0 + 360 / labels.size() * i);
        const Vec2 direction{ Math::Cos(angle), Math::Sin(angle) };

        points << (direction * ratios[i] * 120);

        m_infoLabel(labels[i]).drawAt(direction * 150, ColorF{ m_accentColor, 0.5 });
        Circle{ points[i], 8 }.draw(m_accentColor);
    }

    const Polygon polygon{ points };
    polygon.draw(ColorF{ m_accentColor, 0.5 });
    polygon.drawFrame(4, m_accentColor);
}
...
void StageUI::drawMinimap(Vec2 scroll) const
{
    ...
    // 枠線の図形
    const RoundRect frameRect
    {
        outFrameSize,
        outFrameSize,
        mainSize.x + inFrameSize * 2,
        mainSize.y + inFrameSize * 2,
        frameRounded
    };
    ...
    // 原点を中央に
    Transformer2D mainT(
        Mat3x2::Translate({ frameSize + mainSize.x / 2, frameSize + mainSize.y / 2 }),
        TransformCursor::Yes
    );
    ...
    Rect{
        -mainCenter.x,
        -mainCenter.y - mainCornerR,
        mainSize.x - mainCornerR * 2,
        mainCornerR
    }.draw(Arg::top(m_canvasFadeOutColor), Arg::bottom(m_canvasFadeInColor));
    ...
    Transformer2D scrollT(
        Mat3x2::Translate({ 0, (mainSize.y - decoSize.y - margin) * (ratio - 0.5) }),
        TransformCursor::Yes
    );
